...
public class ShiftColorBySin(float minValue, float maxValue, float speedR, float speedG, float speedB) : SpriteAnimationBase
{
    public override void ApplyState(ref Sprite.Transform spriteTransform, GameTime gameTime)
    {
        ...
        float center = (maxValue + minValue) * 0.5f;
        float offset = (maxValue - minValue) * 0.5f;
